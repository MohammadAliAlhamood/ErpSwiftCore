...
namespace ErpSwiftCore.Application.Features.Billings.InvoiceReporting.Handlers.CommandsHandler
{
    ...
    public class ImportInvoicesFromExcelCommandHandler
        : BaseHandler<ImportInvoicesFromExcelCommand>
    {
        private readonly IInvoiceImportExportCommandService _svc;
        private readonly IMapper _mapper;

        public ImportInvoicesFromExcelCommandHandler(
            IInvoiceImportExportCommandService svc,
            IMapper mapper,
            ILogger<BaseHandler<ImportInvoicesFromExcelCommand>> logger
        ) : base(logger)
        {
            _svc = svc;
            _mapper = mapper;
        ...
        protected override async Task<object?> HandleRequestAsync(
            ImportInvoicesFromExcelCommand req,
            CancellationToken ct)
        {
            using var stream = req.Dto.File.OpenReadStream();
            var result = await _svc.ImportInvoicesFromExcelAsync(stream, ct);
